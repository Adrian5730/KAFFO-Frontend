...
const ItemFavorite = (props) => {
    ...
    return (
        <div className="card-favorite">
            <img src={`images/capsulas/${code}.jpg`} alt={`Imagen de producto ${code}`} />
            <button
                type="button"
            ...
            <div className='info-item-favorite'>
                <span className='price-item-favorite'>Precio: <strong>${price}</strong></span>
                <span className='name-capsule-favorite'>{name}</span>
                <p className='description-item-favorite'>{description.slice(0, 100) + (description.length > 100 ? "..." : "")}</p>
            </div>

            <div className="container-icon-favorite favorite-btn">
                <FavoriteRoundedIcon onClick={handleFavoriteClick} className={claseActiva ? 'mi-clase-activa' : 'icon-favorite'} fontSize="small" />
            </div>
