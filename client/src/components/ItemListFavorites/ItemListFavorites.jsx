...
const ItemListFavorites = () => {
    ...
    useEffect(() => {
        if (products && favorites.length > 0) {
            setfavoritesProducts(products.filter(product => favorites.includes(product.id)));
        }
    }, [products, favorites]);
        ...
        <div className='container-cards'>
            {loading ?
                ...
                favoritesProducts.map(({ id, nombre, codigo, descripcion, precio }) => (
                        <ItemFavorite key={id} name={nombre} code={id} description={descripcion} price={precio} />
                    )) :
